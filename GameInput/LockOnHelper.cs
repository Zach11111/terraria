...
namespace Terraria.GameInput
{
  public class LockOnHelper
  {
    ...
    public static NPC AimedTarget
    {
      get
      {
        return LockOnHelper._pickedTarget == -1 || LockOnHelper._targets.Count < 1 ? (NPC) null : Main.npc[LockOnHelper._targets[LockOnHelper._pickedTarget]];
      }
    }
    ...
    private static void RefreshTargets(Vector2 position, float radius)
    {
      ...
      for (int index = 0; index < Main.npc.Length; ++index)
      {
        ...
        if (LockOnHelper.ValidTarget(n) && (double) n.Distance(position) <= (double) radius && rectangle.Intersects(n.Hitbox) && (double) Lighting.GetSubLight(n.Center).Length() / 3.0 >= 0.029999999329447746)
    ...
    {
      return n != null && n.active && !n.dontTakeDamage && !n.friendly && !n.isLikeATownNPC && n.life >= 1 && !n.immortal && (n.aiStyle != 25 || (double) n.ai[0] != 0.0);
    }
    ...
    public static void Draw(SpriteBatch spriteBatch)
    {
      ...
      Color color1 = Main.OurFavoriteColor.MultiplyRGBA(new Color(0.75f, 0.75f, 0.75f, 1f)) with
      {
        A = 220
      };
      Color favoriteColor = Main.OurFavoriteColor with
      {
        A = 220
      };
      float num1 = (float) (0.93999999761581421 + Math.Sin((double) Main.GlobalTimeWrappedHourly * 6.2831854820251465) * 0.059999998658895493);
      favoriteColor *= num1;
      ...
      for (int i = 0; i < LockOnHelper._drawProgress.GetLength(0); ++i)
      {
        ...
        if ((double) num11 > 0.0)
        {
          ...
          {
            float radians = (float) (6.2831854820251465 / (double) num14 * (double) index + (double) Main.GlobalTimeWrappedHourly * 6.2831854820251465 * 0.25);
            Vector2 vector2_2 = new Vector2(0.0f, (float) (num13 / 2)).RotatedBy((double) radians);
            Vector2 position = Main.ReverseGravitySupport(vector2_1 + vector2_2 - Main.screenPosition);
            float rotation = (float) ((double) radians * ((double) gravDir == 1.0 ? 1.0 : -1.0) + 3.1415927410125732 * ((double) gravDir == 1.0 ? 1.0 : 0.0));

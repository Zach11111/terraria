...
namespace Terraria.Achievements
{
  ...
  public class Achievement
  {
    ...
    {
      ConditionsCompletedTracker tracker = new ConditionsCompletedTracker();
      foreach (KeyValuePair<string, AchievementCondition> condition in this._conditions)
        tracker.AddCondition(condition.Value);
      this.UseTracker((IAchievementTracker) tracker);
    }
    ...
    public void UseConditionsCompletedTracker(params string[] conditions)
    {
      ...
      for (int index = 0; index < conditions.Length; ++index)
      {
        ...
        tracker.AddCondition(this._conditions[condition]);
      }
      this.UseTracker((IAchievementTracker) tracker);
    }

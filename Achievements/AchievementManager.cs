      ...
      lock (AchievementManager._ioLock)
      {
        ...
        try
        {
          using (MemoryStream memoryStream = new MemoryStream(buffer))
          {
            ...
            {
              using (BsonReader reader = new BsonReader((Stream) cryptoStream))
                dictionary = JsonSerializer.Create(this._serializerSettings).Deserialize<Dictionary<string, AchievementManager.StoredAchievement>>((JsonReader) reader);
            }
          }
        }
        catch (Exception ex)

  ...
  public class Framing
  {
    ...
    public static void WallFrame(int i, int j, bool resetFrame = false)
    {
      ...
      else
      {
        ...
        {
          wallFrameNumber = Framing.phlebasTileFrameNumberLookup[j % 4][i % 3] - 1;
          tile1.wallFrameNumber((byte) wallFrameNumber);
        }
        else if (Main.wallLargeFrames[(int) tile1.wall] == (byte) 2)
        {
          wallFrameNumber = Framing.lazureTileFrameNumberLookup[i % 2][j % 2] - 1;
          tile1.wallFrameNumber((byte) wallFrameNumber);
        }
        else if (resetFrame)
        {
          wallFrameNumber = WorldGen.genRand.Next(0, 3);
          if (tile1.wall == (ushort) 21 && WorldGen.genRand.Next(2) == 0)
            wallFrameNumber = 2;
          tile1.wallFrameNumber((byte) wallFrameNumber);
        }
        else
          wallFrameNumber = (int) tile1.wallFrameNumber();
        if (index == 15)
          index += Framing.centerWallFrameLookup[i % 3][j % 3];
        Point16 point16 = Framing.wallFrameLookup[index][wallFrameNumber];
    ...
    public static Tile GetTileSafely(int i, int j)
    {
      if (!WorldGen.InWorld(i, j))
        return new Tile();
      Tile tileSafely = Main.tile[i, j];
      if (tileSafely == null)
      {
        tileSafely = new Tile();
        Main.tile[i, j] = tileSafely;
      }
      return tileSafely;
    }

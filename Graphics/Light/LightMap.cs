...
namespace Terraria.Graphics.Light
{
  public class LightMap
  {
    ...
    private void BlurLine(int startIndex, int endIndex, int stride)
    {
      ...
      bool flag2 = false;
      bool flag3 = false;
      for (int modifier = startIndex; modifier != endIndex + stride; modifier += stride)
      {
        if ((double) zero.X < (double) this._colors[modifier].X)
        {
          zero.X = this._colors[modifier].X;
          flag1 = false;
        }
        else if (!flag1)
        {
          if ((double) zero.X < 0.018500000238418579)
            flag1 = true;
          else
            this._colors[modifier].X = zero.X;
        }
        if ((double) zero.Y < (double) this._colors[modifier].Y)
        {
          zero.Y = this._colors[modifier].Y;
          flag2 = false;
        }
        else if (!flag2)
        {
          if ((double) zero.Y < 0.018500000238418579)
            flag2 = true;
          else
            this._colors[modifier].Y = zero.Y;
        }
        if ((double) zero.Z < (double) this._colors[modifier].Z)
        {
          zero.Z = this._colors[modifier].Z;
          flag3 = false;
        }
        else if (!flag3)
        {
          if ((double) zero.Z < 0.018500000238418579)
            flag3 = true;
          else
            this._colors[modifier].Z = zero.Z;
        }
        if (!(flag1 & flag3 & flag2))
        {
          switch (this._mask[modifier])
          {
            ...
            case LightMaskMode.Water:
              float num = (float) this._random.WithModifier((ulong) modifier).Next(98, 100) / 100f;
              if (!flag1)

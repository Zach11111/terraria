...
namespace Terraria.Graphics.Effects
{
  public class FilterManager : EffectManager<Filter>
  {
      ...
      Color clearColor)
    {
      ...
      RenderTarget2D texture = screenTarget1;
      ...
      if ((double) Main.player[Main.myPlayer].gravDir == -1.0)
      {
        ...
        Main.spriteBatch.Draw((Texture2D) texture, Vector2.Zero, Color.White);
      ...
      for (; linkedListNode != null; linkedListNode = next)
      {
        ...
        if (filter2.Priority >= this._priorityThreshold)
        {
          ...
          if (num > this._activeFilterCount - this._filterLimit && filter2.IsVisible())
          {
            if (filter1 != null)
            {
              RenderTarget2D renderTarget = texture != screenTarget1 ? screenTarget1 : screenTarget2;
              graphicsDevice.SetRenderTarget(renderTarget);
              graphicsDevice.Clear(clearColor);
              Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend);
              filter1.Apply();
              Main.spriteBatch.Draw((Texture2D) texture, Vector2.Zero, Main.ColorOfTheSkies);
              Main.spriteBatch.End();
              texture = texture != screenTarget1 ? screenTarget1 : screenTarget2;
            }
            filter1 = filter2;
          }
        }
    ...
    public bool HasActiveFilter() => this._activeFilters.Count != 0;

  ...
  public class CaptureInterface
  {
    ...
    private class ModeEdgeSelection : CaptureInterface.CaptureInterfaceMode
    {
      ...
      private void DrawCursors(SpriteBatch sb)
      {
        ...
        Main.hslToRgb((float) (((double) hsl.X + 0.33000001311302185) % 1.0), hsl.Y, hsl.Z);
        Main.hslToRgb((float) (((double) hsl.X - 0.33000001311302185) % 1.0), hsl.Y, hsl.Z);
        Color white;
        ...
        else
        {
          ...
          if (!Main.mapFullscreen)
          {
            ...
            Utils.DrawCursorSingle(sb, color, num4 - 1.57079637f, Main.cursorScale * num1, Main.ReverseGravitySupport(vector2_3 - Main.screenPosition), 4, specialMode);
          }
          else
          {
            ...
            if (CaptureInterface.EdgeBPinned)
            {
              ...
              CaptureInterface.GetMapCoords(result1.X, result1.Y, 1, out result1);
            Utils.DrawCursorSingle(sb, color, num3 - 1.57079637f, Main.cursorScale * num1, result1.ToVector2(), 4);
          }
        }
        if (!CaptureInterface.EdgeBPinned)
        {
        ...
        else
        {
          ...
          if (!Main.mapFullscreen)
          {
            ...
            Utils.DrawCursorSingle(sb, white, num8 - 1.57079637f, Main.cursorScale * num1, Main.ReverseGravitySupport(vector2_10 - Main.screenPosition), 5, specialMode);
          }
          else
          {
            ...
            if (CaptureInterface.EdgeAPinned)
            {
              ...
              CaptureInterface.GetMapCoords(result3.X, result3.Y, 1, out result3);
            Utils.DrawCursorSingle(sb, white, num7 - 1.57079637f, Main.cursorScale * num1, result3.ToVector2(), 5);
          }
        }
      }
    }
...
label_46:
    ...
    private class ModeChangeSettings : CaptureInterface.CaptureInterfaceMode
    {
      ...
      {
        Rectangle rect = new Rectangle(0, 0, 224, 170);
        if (CaptureInterface.Settings.ScreenAnchor == 0)
        {
          rect.X = 227 - rect.Width / 2;
          rect.Y = 80;
        }
        return rect;
      }

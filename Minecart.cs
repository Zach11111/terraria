...
namespace Terraria
{
  public static class Minecart
  {
    ...
    {
      return tile != null && tile.active() && tile.type == (ushort) 314 && (tile.frameX == (short) 20 || tile.frameX == (short) 21);
    }
      ...
      Mount.MountDelegatesData delegatesData)
    {
      ...
      while (true)
      {
        int x = (int) ((double) vector2_4.X / 16.0);
        int y = (int) ((double) vector2_4.Y / 16.0);
        int index1 = (int) vector2_4.X % 16 / 2;
        if (flag2)
          num4 = index1;
        bool flag7 = index1 != num4;
        if ((trackState1 == Minecart.TrackState.OnBack || trackState1 == Minecart.TrackState.OnTrack || trackState1 == Minecart.TrackState.OnFront) && x != num2)
        {
          int index2 = trackState1 != Minecart.TrackState.OnBack ? (int) tileTrack.FrontTrack() : (int) tileTrack.BackTrack();
          switch ((double) vector2_3.X >= 0.0 ? Minecart._rightSideConnection[index2] : Minecart._leftSideConnection[index2])
          {
            case 0:
              --y;
              vector2_4.Y -= 2f;
        ...
        if (x != num2 || y != num3)
        {
          if (flag2)
            ...
            flag8 = true;
          tileTrack = Main.tile[x, y];
          if (tileTrack == null)
          {
            tileTrack = new Tile();
            Main.tile[x, y] = tileTrack;
          }
        ...
        if (flag1)
        {
          ...
          int index3 = (int) tileTrack.FrontTrack();
          int index4 = (int) tileTrack.BackTrack();
          int num5 = Minecart._tileHeight[index3][index1];
          switch (num5)
          {
            case -4:
            ...
            case -3:
              if (trackState1 == Minecart.TrackState.OnFront)
              {
                ...
                Matrix matrix = (double) Velocity.X <= 0.0 ? (Minecart._rightSideConnection[index3] != 2 ? Matrix.CreateRotationZ(-0.7853982f) : Matrix.CreateRotationZ(0.7853982f)) : (Minecart._leftSideConnection[index3] != 2 ? Matrix.CreateRotationZ(0.7853982f) : Matrix.CreateRotationZ(-0.7853982f));
                vector2_5 = Vector2.Transform(new Vector2(Velocity.X, 0.0f), matrix) with
                {
                  X = Velocity.X
                };
                flag4 = true;
            ...
            case -2:
              if (trackState1 == Minecart.TrackState.OnFront)
              {
                ...
                {
                  float num6 = (float) (x * 16 + (index1 + 1) * 2) - vector2_4.X;
                  vector2_4.X += num6;
            ...
            case -1:
              ...
              break;
            default:
              float num7 = (float) (y * 16 + num5);
              if (x != num2 && trackState1 == Minecart.TrackState.NoTrack && (double) vector2_4.Y > (double) num7 && (double) vector2_4.Y - (double) num7 < 2.0)
              {
                flag8 = false;
              ...
              if (index4 != -1)
              {
                float num8 = (float) (y * 16 + Minecart._tileHeight[index4][index1]);
            ...
            if (flag7 || (double) vector2_3.Y > 0.0)
            {
              switch (trackState1)
              {
                case Minecart.TrackState.AboveTrack:
                  ...
                  break;
                case Minecart.TrackState.OnTrack:
                  int num9 = Minecart._tileHeight[(int) tileTrack.FrontTrack()][index1];
                  int num10 = Minecart._tileHeight[(int) tileTrack.BackTrack()][index1];
              ...
              if (trackState2 != Minecart.TrackState.OnTrack && trackState2 != Minecart.TrackState.OnFront)
              {
                if (trackState2 == Minecart.TrackState.OnBack)
                  index5 = (int) tileTrack.BackTrack();
              }
              else
                index5 = (int) tileTrack.FrontTrack();
              if (index5 != -1)
              {
                ...
                if (trackState1 == Minecart.TrackState.AboveFront && Minecart._trackType[index5] == 1)
                  flag3 = true;
                vector2_3.Y = 0.0f;
                vector2_4.Y = (float) (y * 16 + Minecart._tileHeight[index5][index1]);
              }
            }
          }
        }
        ...
        {
          int index6 = (int) tileTrack.FrontTrack();
          if (Minecart._trackType[index6] == 2 && (double) lastBoost.X == 0.0 && (double) lastBoost.Y == 0.0)
          {
            lastBoost = new Vector2((float) x, (float) y);
            if (Minecart._boostLeft[index6])
              bitsByte[4] = true;
            else
              bitsByte[5] = true;
          }
...
label_99:
    ...
    {
      int tilesSetLookupIndex = 0;
      if (Main.tile[i - 1, j - 1] != null && Main.tile[i - 1, j - 1].type == (ushort) 314)
        ++tilesSetLookupIndex;
      if (Main.tile[i - 1, j] != null && Main.tile[i - 1, j].type == (ushort) 314)
        tilesSetLookupIndex += 2;
      if (Main.tile[i - 1, j + 1] != null && Main.tile[i - 1, j + 1].type == (ushort) 314)
        tilesSetLookupIndex += 4;
      if (Main.tile[i + 1, j - 1] != null && Main.tile[i + 1, j - 1].type == (ushort) 314)
        tilesSetLookupIndex += 8;
      if (Main.tile[i + 1, j] != null && Main.tile[i + 1, j].type == (ushort) 314)
        tilesSetLookupIndex += 16;
      if (Main.tile[i + 1, j + 1] != null && Main.tile[i + 1, j + 1].type == (ushort) 314)
        tilesSetLookupIndex += 32;
      return tilesSetLookupIndex;
    }
      ...
      Mount.MountDelegatesData delegatesData)
    {
      ...
      float num1 = y1 / x;
      double num2 = (double) leftWheel.Y + ((double) Position.X - (double) leftWheel.X) * (double) num1;
      float num3 = (Position.X - (float) (int) Position.X) * num1;
      rotation = (float) Math.Atan2((double) y1, (double) x);
      double y2 = (double) Position.Y;
      return (float) (num2 - y2) + num3;
    }
    ...
    public static void HitTrackSwitch(Vector2 Position, int Width, int Height)
    {
      ...
      int num1 = (int) ((double) magnetPosition.X / 16.0);
      int num2 = (int) ((double) magnetPosition.Y / 16.0);
      Wiring.HitSwitch(num1, num2);
      NetMessage.SendData(59, number: num1, number2: (float) num2);
    }
    ...
    public static Rectangle GetSourceRect(int frameID, int animationFrame = 0)
    {
      ...
      Rectangle sourceRect = new Rectangle((int) vector2.X, (int) vector2.Y, 16, 16);
      if (frameID < 36 && Minecart._trackType[frameID] == 2)
        sourceRect.Y += 18 * animationFrame;
      return sourceRect;
    }
      ...
      int? expectedYOffsetForRight)
    {
      ...
      {
        short index = tileTrack.FrontTrack();
        int num = Minecart.ConvertOffsetYToTrackConnectionValue(expectedYOffsetForLeft.Value);
        if (Minecart._leftSideConnection[(int) index] != num)
          return false;
      }
      if (expectedYOffsetForRight.HasValue)
      {
        short index = tileTrack.FrontTrack();
        int num = Minecart.ConvertOffsetYToTrackConnectionValue(expectedYOffsetForRight.Value);
        if (Minecart._rightSideConnection[(int) index] != num)
          return false;
      }
      return true;
    }

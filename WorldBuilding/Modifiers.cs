...
namespace Terraria.WorldBuilding
{
  public static class Modifiers
  {
    ...
    public class InShape : GenAction
    {
      ...
      public override bool Apply(Point origin, int x, int y, params object[] args)
      {
        return !this._shapeData.Contains(x - origin.X, y - origin.Y) ? this.Fail() : this.UnitApply(origin, x, y, args);
      }
    }
    ...
    public class IsEmpty : GenAction
    {
      public override bool Apply(Point origin, int x, int y, params object[] args)
      {
        return !GenBase._tiles[x, y].active() ? this.UnitApply(origin, x, y, args) : this.Fail();
      }
    }
    ...
    public class IsSolid : GenAction
    {
      public override bool Apply(Point origin, int x, int y, params object[] args)
      {
        return GenBase._tiles[x, y].active() && WorldGen.SolidOrSlopedTile(x, y) ? this.UnitApply(origin, x, y, args) : this.Fail();
      }
    }
    ...
    public class RectangleMask : GenAction
    {
      ...
      public override bool Apply(Point origin, int x, int y, params object[] args)
      {
        return x >= this._xMin + origin.X && x <= this._xMax + origin.X && y >= this._yMin + origin.Y && y <= this._yMax + origin.Y ? this.UnitApply(origin, x, y, args) : this.Fail();
      }
    }
    ...
    public class Dither : GenAction
    {
      ...
      public override bool Apply(Point origin, int x, int y, params object[] args)
      {
        return GenBase._random.NextDouble() >= this._failureChance ? this.UnitApply(origin, x, y, args) : this.Fail();
      }
    }

...
namespace Terraria.DataStructures
{
    ...
    where U : IEntryFilter<T>
  {
    ...
    {
      this.AvailableFilters.AddRange((IEnumerable<U>) filters);
    }
    ...
    {
      return this.AvailableFilters.IndexInRange<U>(filterIndex) && this.ActiveFilters.Contains(this.AvailableFilters[filterIndex]);
    }

    public void SetSearchFilterObject<Z>(Z searchFilter) where Z : ISearchFilter<T>, U
    {
      this._searchFilterFromConstructor = (ISearchFilter<T>) searchFilter;
    }
    ...
    {
      return Language.GetTextValueWith("BestiaryInfo.Filters", (object) new
      {

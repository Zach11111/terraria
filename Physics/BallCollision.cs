...
namespace Terraria.Physics
{
  public static class BallCollision
  {
      ...
      IBallContactListener listener)
    {
      ...
      if (flag && (double) vector2_2.X > -0.0099999997764825821 && (double) vector2_2.X < 0.0099999997764825821 && (double) vector2_2.Y <= 0.0 && (double) vector2_2.Y > -(double) physicsProperties.Gravity)
    ...
    {
      return (double) position.X > 32.0 && (double) position.Y > 32.0 && (double) position.X + (double) size.X < (double) Main.maxTilesX * 16.0 - 32.0 && (double) position.Y + (double) size.Y < (double) Main.maxTilesY * 16.0 - 32.0;
    }
      ...
      ref float distanceSquaredOut)
    {
      ...
      bool collisionPointForTile = false;
      ...
      if (BallCollision.GetTopOrBottomEdge(edgesToTest, x, y, tilePosition, ref edge))
      {
        ...
        if (!collisionPointForTile || (double) num < (double) distanceSquaredOut)
        {

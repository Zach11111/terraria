...
namespace Terraria.UI
{
  public class ChestUI
  {
    ...
    private static void DrawName(SpriteBatch spritebatch)
    {
      ...
      Color baseColor = (Color.White * (float) (1.0 - ((double) byte.MaxValue - (double) Main.mouseTextColor) / (double) byte.MaxValue * 0.5)) with
      {
        A = byte.MaxValue
      };
      int lineAmount;
    ...
    private static void DrawButton(SpriteBatch spriteBatch, int ID, int X, int Y)
    {
      ...
      else
      {
        ...
        Color baseColor = (Color.White * 0.97f * (float) (1.0 - ((double) byte.MaxValue - (double) Main.mouseTextColor) / (double) byte.MaxValue * 0.5)) with
        {
          A = byte.MaxValue
        };
        ...
        if (player.inventory[index1].stack > 0 && player.inventory[index1].type > 0 && !player.inventory[index1].favorited)
        {
          if (player.inventory[index1].maxStack > 1)
          {
            for (int number2 = 0; number2 < 40; ++number2)
            {
              ...
              {
                Chest chest = Main.chest[player.chest];
                if (chest.item[number2].stack < chest.item[number2].maxStack && player.inventory[index1].IsTheSameAs(chest.item[number2]))
                {
                  int num = player.inventory[index1].stack;
                  if (player.inventory[index1].stack + chest.item[number2].stack > chest.item[number2].maxStack)
                    num = chest.item[number2].maxStack - chest.item[number2].stack;
                  ...
                  if (chest.item[number2].type == 0)
                  {
                    chest.item[number2] = player.inventory[index1].Clone();
                    player.inventory[index1].SetDefaults();
                  }
                  if (Main.netMode == 1)
                    NetMessage.SendData(32, number: player.chest, number2: (float) number2);
              ...
              {
                if (player.bank2.item[number2].stack < player.bank2.item[number2].maxStack && player.inventory[index1].IsTheSameAs(player.bank2.item[number2]))
                {
                  int num = player.inventory[index1].stack;
                  if (player.inventory[index1].stack + player.bank2.item[number2].stack > player.bank2.item[number2].maxStack)
                    num = player.bank2.item[number2].maxStack - player.bank2.item[number2].stack;
                  player.inventory[index1].stack -= num;
                  player.bank2.item[number2].stack += num;
                  SoundEngine.PlaySound(7);
                  if (player.inventory[index1].stack <= 0)
                  {
                    player.inventory[index1].SetDefaults();
                  ...
                  if (player.bank2.item[number2].type == 0)
                  {
                    player.bank2.item[number2] = player.inventory[index1].Clone();
                    player.inventory[index1].SetDefaults();
                  }
                }
              }
              ...
              {
                if (player.bank3.item[number2].stack < player.bank3.item[number2].maxStack && player.inventory[index1].IsTheSameAs(player.bank3.item[number2]))
                {
                  int num = player.inventory[index1].stack;
                  if (player.inventory[index1].stack + player.bank3.item[number2].stack > player.bank3.item[number2].maxStack)
                    num = player.bank3.item[number2].maxStack - player.bank3.item[number2].stack;
                  player.inventory[index1].stack -= num;
                  player.bank3.item[number2].stack += num;
                  SoundEngine.PlaySound(7);
                  if (player.inventory[index1].stack <= 0)
                  {
                    player.inventory[index1].SetDefaults();
                  ...
                  if (player.bank3.item[number2].type == 0)
                  {
                    player.bank3.item[number2] = player.inventory[index1].Clone();
                    player.inventory[index1].SetDefaults();
                  }
                }
              }
              ...
              {
                if (player.bank4.item[number2].stack < player.bank4.item[number2].maxStack && player.inventory[index1].IsTheSameAs(player.bank4.item[number2]))
                {
                  int num = player.inventory[index1].stack;
                  if (player.inventory[index1].stack + player.bank4.item[number2].stack > player.bank4.item[number2].maxStack)
                    num = player.bank4.item[number2].maxStack - player.bank4.item[number2].stack;
                  player.inventory[index1].stack -= num;
                  player.bank4.item[number2].stack += num;
                  SoundEngine.PlaySound(7);
                  if (player.inventory[index1].stack <= 0)
                  {
                    player.inventory[index1].SetDefaults();
                  ...
                  if (player.bank4.item[number2].type == 0)
                  {
                    player.bank4.item[number2] = player.inventory[index1].Clone();
                    player.inventory[index1].SetDefaults();
                  }
                }
              }
              else if (player.bank.item[number2].stack < player.bank.item[number2].maxStack && player.inventory[index1].IsTheSameAs(player.bank.item[number2]))
              {
                int num = player.inventory[index1].stack;
                if (player.inventory[index1].stack + player.bank.item[number2].stack > player.bank.item[number2].maxStack)
                  num = player.bank.item[number2].maxStack - player.bank.item[number2].stack;
                player.inventory[index1].stack -= num;
                player.bank.item[number2].stack += num;
                SoundEngine.PlaySound(7);
                if (player.inventory[index1].stack <= 0)
                {
                  player.inventory[index1].SetDefaults();
                ...
                if (player.bank.item[number2].type == 0)
                {
                  player.bank.item[number2] = player.inventory[index1].Clone();
                  player.inventory[index1].SetDefaults();
                }
              }
            }
          ...
          if (player.inventory[index1].stack > 0)
          {
            if (player.chest > -1)
            {
              for (int number2 = 0; number2 < 40; ++number2)
              {
                if (Main.chest[player.chest].item[number2].stack == 0)
                {
                  SoundEngine.PlaySound(7);
                  Main.chest[player.chest].item[number2] = player.inventory[index1].Clone();
                  player.inventory[index1].SetDefaults();
                  if (Main.netMode == 1)
                  {
                    NetMessage.SendData(32, number: player.chest, number2: (float) number2);
            ...
            else if (player.chest == -4)
            {
              ...
              {
                if (player.bank3.item[index3].stack == 0)
                {
                  SoundEngine.PlaySound(7);
                  player.bank3.item[index3] = player.inventory[index1].Clone();
                  player.inventory[index1].SetDefaults();
                  break;
                }
              }
            ...
            else if (player.chest == -5)
            {
              ...
              {
                if (player.bank4.item[index4].stack == 0)
                {
                  SoundEngine.PlaySound(7);
                  player.bank4.item[index4] = player.inventory[index1].Clone();
                  player.inventory[index1].SetDefaults();
                  break;
                }
              }
            ...
            else
            {
              ...
              {
                if (player.bank.item[index5].stack == 0)
                {
                  SoundEngine.PlaySound(7);
                  player.bank.item[index5] = player.inventory[index1].Clone();
                  player.inventory[index1].SetDefaults();
                  break;
                }
              }
    ...
    public static void QuickStack(ContainerTransferContext context, bool voidStack = false)
    {
      ...
      {
        for (int number2 = 0; number2 < flagArray.Length; ++number2)
          NetMessage.SendData(32, number: player.chest, number2: (float) number2);
      }
      intList1.Clear();
      intList2.Clear();
    ...
    public static void Restock()
    {
      ...
      bool flag1 = false;
      for (int number2 = 0; number2 < objArray.Length; ++number2)
      {
        if (objArray[number2].stack >= 1 && objArray[number2].prefix == (byte) 0 && intSet.Contains(objArray[number2].netID))
        {
          bool flag2 = false;
          for (int index = 0; index < intList1.Count; ++index)
          {
            int slot = intList1[index];
            int context = 0;
            if (slot >= 50)
              context = 2;
            if (inventory[slot].netID == objArray[number2].netID && ItemSlot.PickItemMovementAction(inventory, context, slot, objArray[number2]) != -1)
            {
              int num = objArray[number2].stack;
          ...
          if (!flag2 && intList2.Count > 0 && objArray[number2].ammo != 0)
          {
            for (int index = 0; index < intList2.Count; ++index)
            {
              int context = 0;
              if (intList2[index] >= 50)
                context = 2;
              if (ItemSlot.PickItemMovementAction(inventory, context, intList2[index], objArray[number2]) != -1)
              {
                Utils.Swap<Item>(ref inventory[intList2[index]], ref objArray[number2]);
                if (Main.netMode == 1 && Main.player[Main.myPlayer].chest > -1)
                  NetMessage.SendData(32, number: Main.player[Main.myPlayer].chest, number2: (float) number2);
    ...
    public static long MoveCoins(Item[] pInv, Item[] cInv, ContainerTransferContext context)
    {
      ...
      for (int index1 = 0; index1 < 40; ++index1)
      {
        if (numArray2[index1] >= 0 && cInv[index1].type == 0)
        {
          ...
          {
            cInv[number2].SetDefaults(71 + index2);
            cInv[number2].stack = numArray1[index2];
            if (cInv[number2].stack > cInv[number2].maxStack)
              cInv[number2].stack = cInv[number2].maxStack;
            numArray1[index2] -= cInv[number2].stack;
            numArray2[index1] = -1;
          }
          if (Main.netMode == 1 && Main.player[Main.myPlayer].chest > -1)
            NetMessage.SendData(32, number: Main.player[Main.myPlayer].chest, number2: (float) number2);
          intList2.Remove(number2);
        }
      }
      for (int index3 = 0; index3 < 40; ++index3)
      {
        if (numArray2[index3] >= 0 && cInv[index3].type == 0)
        {
          int number2 = index3;
          int index4 = 3;
          while (index4 >= 0)
          {
            ...
            {
              cInv[number2].SetDefaults(71 + index4);
              cInv[number2].stack = numArray1[index4];
              if (cInv[number2].stack > cInv[number2].maxStack)
                cInv[number2].stack = cInv[number2].maxStack;
              numArray1[index4] -= cInv[number2].stack;
              numArray2[index3] = -1;
              break;
            }
            if (numArray1[index4] == 0)
      ...
      int index7 = 3;
      while (index7 >= 0 && intList1.Count > 0)
      {
        int index8 = intList1[0];
        if (numArray1[index7] > 0)
        {
          pInv[index8].SetDefaults(71 + index7);
          pInv[index8].stack = numArray1[index7];
          if (pInv[index8].stack > pInv[index8].maxStack)
            pInv[index8].stack = pInv[index8].maxStack;
          numArray1[index7] -= pInv[index8].stack;
          flag1 = false;
          intList1.RemoveAt(0);
        }
        if (numArray1[index7] == 0)
    ...
    public static bool TryPlacingInChest(Item I, bool justCheck, int itemSlotContext)
    {
      ...
      if (I.maxStack > 1)
      {
        for (int number2 = 0; number2 < 40; ++number2)
        {
          if (chestinv[number2].stack < chestinv[number2].maxStack && I.IsTheSameAs(chestinv[number2]))
          {
            int num = I.stack;
            if (I.stack + chestinv[number2].stack > chestinv[number2].maxStack)
              num = chestinv[number2].maxStack - chestinv[number2].stack;
            if (justCheck)
            {
              flag = flag || num > 0;
              break;

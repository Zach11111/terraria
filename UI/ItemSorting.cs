...
namespace Terraria.UI
{
  public class ItemSorting
  {
    ...
    private static void Sort(Item[] inv, params int[] ignoreSlots)
    {
      ...
      for (int index = 0; index < inv.Length; ++index)
      {
        if (!((IEnumerable<int>) ignoreSlots).Contains<int>(index))
        {
          ...
          if (obj != null && obj.stack != 0 && obj.type != 0 && !obj.favorited)
            collection1.Add(index);
        }
      }
      for (int index1 = 0; index1 < collection1.Count; ++index1)
      {
        Item obj1 = inv[collection1[index1]];
        if (obj1.stack < obj1.maxStack)
        {
          int num1 = obj1.maxStack - obj1.stack;
          for (int index2 = index1; index2 < collection1.Count; ++index2)
          {
            if (index1 != index2)
            {
              Item obj2 = inv[collection1[index2]];
              if (obj1.type == obj2.type && obj2.stack != obj2.maxStack)
              {
                ...
                if (obj2.stack == 0)
                {
                  inv[collection1[index2]] = new Item();
                  collection1.Remove(collection1[index2]);
                  --index1;
                  int num3 = index2 - 1;
                  break;
                }
      ...
      List<int> intList1 = new List<int>((IEnumerable<int>) collection1);
      for (int index = 0; index < inv.Length; ++index)
      {
        if (!((IEnumerable<int>) ignoreSlots).Contains<int>(index) && !intList1.Contains(index))
        {
          Item obj = inv[index];
      ...
      {
        List<int> collection2 = layer.SortingMethod(layer, inv, collection1);
        if (collection2.Count > 0)
          intList3.Add(collection2.Count);
        intList2.AddRange((IEnumerable<int>) collection2);
      }
      intList2.AddRange((IEnumerable<int>) collection1);
      List<Item> objList = new List<Item>();
      ...
      float num = 1f / (float) intList3.Count;
      float hue = num / 2f;
      for (int index3 = 0; index3 < objList.Count; ++index3)
      {
        int index4 = intList1[0];
    ...
    public static void SortChest()
    {
      ...
      {
        if (tupleArray2[number2] != tupleArray1[number2])

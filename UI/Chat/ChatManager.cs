...
namespace Terraria.UI.Chat
{
  public static class ChatManager
  {
    ...
    public static List<TextSnippet> ParseMessage(string text, Color baseColor)
    {
      ...
      List<TextSnippet> message = new List<TextSnippet>();
      int startIndex = 0;
      foreach (Match match in matchCollection)
      {
        if (match.Index > startIndex)
          message.Add(new TextSnippet(text.Substring(startIndex, match.Index - startIndex), baseColor));
        ...
        if (handler != null)
        {
          message.Add(handler.Parse(text1, baseColor, options));
          message[message.Count - 1].TextOriginal = match.ToString();
        }
        else
          message.Add(new TextSnippet(text1, baseColor));
      }
      if (text.Length > startIndex)
        message.Add(new TextSnippet(text.Substring(startIndex, text.Length - startIndex), baseColor));
      return message;
    }
      ...
      float maxWidth = -1f)
    {
      ...
      for (int index1 = 0; index1 < snippets.Length; ++index1)
      {
        ...
        if (snippet.UniqueDraw(true, out size, (SpriteBatch) null))
        {
          ...
          stringSize.Y = Math.Max(stringSize.Y, minimum.Y + size.Y);
        }
        else
        {
          ...
          foreach (string str in strArray1)
          {
            ...
            for (int index2 = 0; index2 < strArray2.Length; ++index2)
            {
              ...
              if ((double) num1 < (double) scale)
                num1 = scale;
              Vector2 vector2 = font.MeasureString(strArray2[index2]);
              vec.Between(minimum, minimum + vector2);
              minimum.X += vector2.X * baseScale.X * scale;
              stringSize.X = Math.Max(stringSize.X, minimum.X);
              stringSize.Y = Math.Max(stringSize.Y, minimum.Y + vector2.Y);
            }

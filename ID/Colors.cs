...
namespace Terraria.ID
{
  public static class Colors
  {
    ...
    public static Color CurrentLiquidColor
    {
      get
      {
        Color currentLiquidColor = Color.Transparent;
        bool flag = true;
        for (int index = 0; index < 11; ++index)
        {
          if ((double) Main.liquidAlpha[index] > 0.0)
          {
            ...
            {
              flag = false;
              currentLiquidColor = Colors._liquidColors[index];
            }
            else
              currentLiquidColor = Color.Lerp(currentLiquidColor, Colors._liquidColors[index], Main.liquidAlpha[index]);
          }
        }
        return currentLiquidColor;
      }
    }

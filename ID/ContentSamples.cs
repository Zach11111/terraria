...
namespace Terraria.ID
{
  public static class ContentSamples
  {
    ...
    private static void ModifyNPCIds()
    {
      ...
      short key1 = 499;
      for (int key2 = 498; key2 <= 506; ++key2)
        creditIdsByNpcNetIds[key2] = creditIdsByNpcNetIds[(int) key1];
    ...
    {
      float rarityStarsCount = 1f + (float) npc.rarity;
      if (npc.rarity == 1)
        ++rarityStarsCount;
      else if (npc.rarity == 2)
        rarityStarsCount += 1.5f;
      else if (npc.rarity == 3)
        rarityStarsCount += 2f;
      else if (npc.rarity == 4)
        rarityStarsCount += 2.5f;
      else if (npc.rarity == 5)
        rarityStarsCount += 3f;
      else if (npc.rarity > 0)
        rarityStarsCount += 3.5f;
      if (npc.boss)
        rarityStarsCount += 0.5f;
      ...
      if (num > 10000)
        ...
        ++rarityStarsCount;
      if ((double) rarityStarsCount > 5.0)
        rarityStarsCount = 5f;
      return (int) rarityStarsCount;
    }
    ...
    public static class BestiaryHelper
    {
      ...
      {
        int bestiaryTownPriority = NPCID.Sets.TownNPCBestiaryPriority.IndexOf(npc.netID);
        if (bestiaryTownPriority == -1)
          bestiaryTownPriority = int.MaxValue;
        return bestiaryTownPriority;
      }

      public static int GetBestiaryNormalGoldCritterPriority(NPC npc)
      {
        int goldCritterPriority = NPCID.Sets.NormalGoldCritterBestiaryPriority.IndexOf(npc.netID);
        if (goldCritterPriority == -1)
          goldCritterPriority = int.MaxValue;
        return goldCritterPriority;
      }

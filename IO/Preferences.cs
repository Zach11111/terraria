      ...
      lock (this._lock)
      {
        ...
        try
        {
          ...
          else
          {
            ...
            {
              using (BsonReader reader = new BsonReader((Stream) fileStream))
                this._data = JsonSerializer.Create(this._serializerSettings).Deserialize<Dictionary<string, object>>((JsonReader) reader);
            }
          }
          if (this._data == null)

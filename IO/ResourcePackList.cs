...
namespace Terraria.IO
{
  public class ResourcePackList
  {
    ...
    public IEnumerable<ResourcePack> AllPacks
    {
      get
      {
        return (IEnumerable<ResourcePack>) this._resourcePacks.OrderBy<ResourcePack, string>((Func<ResourcePack, string>) (pack => pack.Name)).ThenBy<ResourcePack, ResourcePackVersion>((Func<ResourcePack, ResourcePackVersion>) (pack => pack.Version)).ThenBy<ResourcePack, string>((Func<ResourcePack, string>) (pack => pack.FileName));
      }
    }
    ...
    {
      List<ResourcePackList.ResourcePackEntry> o = new List<ResourcePackList.ResourcePackEntry>(this._resourcePacks.Count);
      o.AddRange(this._resourcePacks.Select<ResourcePack, ResourcePackList.ResourcePackEntry>((Func<ResourcePack, ResourcePackList.ResourcePackEntry>) (pack => new ResourcePackList.ResourcePackEntry(pack.FileName, pack.IsEnabled, pack.SortingOrder))));
      return JArray.FromObject((object) o);
    }
      ...
      JArray serializedState)
    {
      try
      {
        ...
        {
          if (serializedState.Count != 0)
            return (IEnumerable<ResourcePackList.ResourcePackEntry>) serializedState.ToObject<List<ResourcePackList.ResourcePackEntry>>();
        }
      }

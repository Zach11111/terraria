...
namespace Terraria.Testing
{
  public class PacketHistory
  {
    ...
    public void Dump(string reason)
    {
      ...
      Buffer.BlockCopy((Array) this._buffer, this._bufferPosition, (Array) dst, 0, this._buffer.Length - this._bufferPosition);
      Buffer.BlockCopy((Array) this._buffer, 0, (Array) dst, this._buffer.Length - this._bufferPosition, this._bufferPosition);
    ...
    private string CreateDumpFileName()
    {
      ...
      return string.Format("Net_{0}_{1}_{2}_{3}.txt", Main.dedServ ? (object) "TerrariaServer" : (object) "Terraria", (object) Main.versionNumber, (object) localTime.ToString("MM-dd-yy_HH-mm-ss-ffff", (IFormatProvider) CultureInfo.InvariantCulture), (object) Thread.CurrentThread.ManagedThreadId);
    }

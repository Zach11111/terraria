...
namespace Terraria.Initializers
{
  public class UILinksInitializer
  {
    ...
    public static void Load()
    {
      ...
      cp2.UpdateEvent += (Action) (() =>
      {
        ...
        for (int key = 40; key <= 49; ++key)
          cp2.LinkMap[key].Down = !inReforgeMenu ? (!flag1 ? (!flag2 ? (key != 40 ? -2 : 311) : 2700 + key - 40) : 400 + key - 40) : (key < 45 ? 303 : 304);
        if (flag4)
        {
          for (int key = 40; key <= 47; ++key)
            cp2.LinkMap[key].Down = 5100 + key - 40;
        }
        if (flag3)
        {
          for (int key = 44; key <= 45; ++key)
            cp2.LinkMap[key].Down = 5000 + key - 44;
        }
        if (flag1)
        {
      ...
      page5.PageOnLeft = 0;
      page5.PageOnRight = 0;
      UILinkPointNavigator.RegisterPage(page5, 13);
      page5.IsValidEvent += (Func<bool>) (() => Main.playerInventory && Main.npcShop != 0);
      ...
      cp10.LinkMap[1501].OnSpecialInteracts += (Func<string>) (() =>
      {
        ...
        if (Main.mouseItem.type == 0 && createItem.maxStack > 1 && player.ItemSpace(createItem).CanTakeItemToPersonalInventory && !player.HasLockedInventory())
        {
          ...
          if (UILinksInitializer.CanExecuteInputCommand() && PlayerInput.Triggers.Current.Grapple && Main.stackSplit <= 1)
          {
            ...
            if (UILinksInitializer.SomeVarsForUILinkers.SequencedCraftingCurrent == Main.recipe[Main.availableRecipe[Main.focusRecipe]])
            {
              ...
              Main.mouseItem = player.GetItem(player.whoAmI, Main.mouseItem, new GetItemSettings(NoText: true));
            }
          }
        }
        ...
        if (flag8 || Main.mouseItem.type == createItem.type && Main.mouseItem.stack < Main.mouseItem.maxStack)
        {
          if (flag8)
      ...
      for (int index1 = 700; index1 < 1500; ++index1)
      {
        ...
        uiLinkPoint.OnSpecialInteracts += (Func<string>) (() =>
        {
          ...
          {
            int index2 = Main.recStart + IHateLambda - 700;
            if (Main.mouseItem.type == 0 && Main.recipe[Main.availableRecipe[index2]].createItem.maxStack > 1 && player.ItemSpace(Main.recipe[Main.availableRecipe[index2]].createItem).CanTakeItemToPersonalInventory && !player.HasLockedInventory())
            {
              ...
              if (UILinksInitializer.CanExecuteInputCommand() && PlayerInput.Triggers.Current.Grapple && Main.stackSplit <= 1)
              {
                ...
                {
                  Main.CraftItem(Main.recipe[Main.availableRecipe[index2]]);
                  Main.mouseItem = player.GetItem(player.whoAmI, Main.mouseItem, GetItemSettings.InventoryUIToInventorySettings);
                }
              }
            }
    ...
    private static int GetCornerWrapPageIdFromLeftToRight() => 8;
    ...
    {
      if (Main.MenuUI.CurrentState is UICharacterCreation currentState1)
        currentState1.TryMovingCategory(direction);
    ...
    public static void FancyExit()
    {
      switch (UILinkPointNavigator.Shortcuts.BackButtonCommand)
      {
        ...
        case 6:
          ...
          break;
        case 7:
          if (!(Main.MenuUI.CurrentState is IHaveBackButtonCommand currentState))
            break;
          currentState.HandleBackButtonUsage();
          break;
      }
    }

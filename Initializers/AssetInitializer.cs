...
namespace Terraria.Initializers
{
  public static class AssetInitializer
  {
    ...
    {
      AssetReaderCollection provider1 = new AssetReaderCollection();
      provider1.RegisterReader((IAssetReader) new PngReader(XnaExtensions.Get<IGraphicsDeviceService>((IServiceProvider) services).GraphicsDevice), new string[1]
      {
        ".png"
      });
      provider1.RegisterReader((IAssetReader) new XnbReader((IServiceProvider) services), new string[1]
      {
        ".xnb"
      });
      AsyncAssetLoader asyncAssetLoader = new AsyncAssetLoader(provider1, 20);
      ...
      IAssetRepository provider2 = (IAssetRepository) new AssetRepository((IAssetLoader) new AssetLoader(provider1), (IAsyncAssetLoader) asyncAssetLoader);
      services.AddService(typeof (AssetReaderCollection), (object) provider1);
      services.AddService(typeof (IAssetRepository), (object) provider2);
    }
    ...
    {
      SoundEngine.Load((IServiceProvider) Main.instance.Services);
    }
    ...
    private static void LoadTextures(AssetRequestMode mode)
    {
      ...
      for (int index = 0; index < TextureAssets.AccBeard.Length; ++index)
        TextureAssets.AccBeard[index] = AssetInitializer.LoadAsset<Texture2D>("Images/Acc_Beard_" + (object) index, (AssetRequestMode) 0);
      ...
      for (int index = 0; index < (int) GlowMaskID.Count; ++index)
      ...
      for (int index = 0; index < (int) ExtrasID.Count; ++index)
      ...
      for (int index = 0; index < CloudID.Count; ++index)
      ...
      for (int index = 0; index < TextureAssets.CageTop.Length; ++index)
        TextureAssets.CageTop[index] = AssetInitializer.LoadAsset<Texture2D>("Images\\CageTop_" + (object) index, mode);
      ...
      for (int index = 0; index < (int) ChainID.Count; ++index)

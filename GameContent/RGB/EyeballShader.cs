...
namespace Terraria.GameContent.RGB
{
  public class EyeballShader : ChromaShader
  {
    ...
    public virtual void Update(float elapsedTime)
    {
      ...
      if (num1 == 0)
        return;
      float num2 = (float) this._random.NextDouble() * 6.28318548f;
      float num3;
      if (this._isSpawning)
      {
        this._timeUntilPupilMove = (float) (this._random.NextDouble() * 0.40000000596046448 + 0.30000001192092896);
        num3 = (float) this._random.NextDouble() * 0.7f;
      }
      else
      {
        this._timeUntilPupilMove = (float) (this._random.NextDouble() * 0.40000000596046448 + 0.60000002384185791);
    ...
    private void UpdateEyelid(float elapsedTime)
    {
      ...
      switch (this._eyelidState)
      {
        ...
        case EyeballShader.EyelidState.Opening:
          ...
          if ((double) this._eyelidStateTime <= 0.40000000596046448)
            break;
          this._eyelidStateTime = 0.0f;
        ...
        case EyeballShader.EyelidState.Closing:
          this._eyelidProgress = (float) (1.0 - (double) this._eyelidStateTime / 0.40000000596046448);
          if ((double) this._eyelidStateTime <= 0.40000000596046448)
            break;
          this._eyelidStateTime = 0.0f;
          this._eyelidState = EyeballShader.EyelidState.Closed;
      ...
      float time)
    {
      ...
      for (int index1 = 0; index1 < fragment.Count; ++index1)
      {
        ...
        float num2 = (float) Math.Sqrt(1.0 - 0.40000000596046448 * (double) vector2_3.Y * (double) vector2_3.Y) * 5f;
        float num3 = Math.Abs(vector2_3.X) - num2 * (float) (1.1000000238418579 * (double) this._eyelidProgress - 0.10000000149011612);
        if ((double) num3 > 0.0)

  ...
  public class DD2Shader : ChromaShader
  {
      ...
      float time)
    {
      ...
      for (int index = 0; index < fragment.Count; ++index)
      {
        ...
        if ((double) num3 < 0.0)
          ++num3;
        float num4 = (double) num3 <= 0.800000011920929 ? num3 / 0.8f : num3 * (float) (1.0 - ((double) num3 - 1.0 + 0.20000000298023224) / 0.20000000298023224);
        ...
        if ((double) num1 < 0.5)
        {
          float amount2 = 1f - MathHelper.Clamp((float) (((double) num1 - 0.5 + 0.40000000596046448) / 0.40000000596046448), 0.0f, 1f);

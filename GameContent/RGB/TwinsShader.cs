...
namespace Terraria.GameContent.RGB
{
  public class TwinsShader : ChromaShader
  {
      ...
      float time)
    {
      ...
      {
        bool flag = true;
        float num1 = (float) ((double) time * 0.10000000149011612 % 2.0);
        if ((double) num1 > 1.0)
        {
          num1 = 2f - num1;
          flag = false;
        ...
        for (int index = 0; index < fragment.Count; ++index)
        {
          ...
          if ((double) num2 < 0.5)
          {
            float amount1 = 1f - MathHelper.Clamp((float) (((double) num2 - 0.5 + 0.20000000298023224) / 0.20000000298023224), 0.0f, 1f);
            float amount2 = MathHelper.Clamp((float) (((double) vector2_2.X + 0.5 - 0.20000000298023224) / 0.60000002384185791), 0.0f, 1f);
            if (flag)
              amount2 = 1f - amount2;
            ...
            if (!flag && 3.1415927410125732 - (double) Math.Abs(num3) < 0.60000002384185791)
          ...
          else if (!flag)
          {
            ...
            if ((double) num5 < 0.699999988079071)
            {
              ...
              float amount = dynamicNoise * dynamicNoise * dynamicNoise * (1f - MathHelper.Clamp((float) (((double) num5 - 0.699999988079071 + 0.30000001192092896) / 0.30000001192092896), 0.0f, 1f));

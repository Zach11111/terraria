...
namespace Terraria.GameContent.RGB
{
  public class EyeOfCthulhuShader : ChromaShader
  {
      ...
      float time)
    {
      ...
      {
        float num1 = (float) ((double) time * 0.20000000298023224 % 2.0);
        int num2 = 1;
        if ((double) num1 > 1.0)
        {
          num1 = 2f - num1;
        ...
        for (int index = 0; index < fragment.Count; ++index)
        {
          ...
          if ((double) num3 < 0.5)
          {
            float amount1 = 1f - MathHelper.Clamp((float) (((double) num3 - 0.5 + 0.20000000298023224) / 0.20000000298023224), 0.0f, 1f);
            float amount2 = MathHelper.Clamp((float) (((double) vector2_3.X + 0.5 - 0.20000000298023224) / 0.60000002384185791), 0.0f, 1f);
            if (num2 == 1)
              amount2 = 1f - amount2;

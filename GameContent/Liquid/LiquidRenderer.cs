...
namespace Terraria.GameContent.Liquid
{
  public class LiquidRenderer
  {
            ...
            for (int index14 = 2; index14 < rectangle.Width - 2; ++index14)
            {
              for (int index15 = 2; index15 < rectangle.Height - 2; ++index15)
              {
                if (liquidCachePtr9->HasVisibleLiquid)
                {
                  ...
                  if (liquidCachePtr9->IsHalfBrick && liquidCachePtr9->IsSolid && (double) num12 > 0.5)
                    num12 = 0.5f;
    ...
    {
      return (float) Math.Sin((((double) worldPositionX + (double) worldPositionY / 6.0) / 10.0 - Main.timeForVisualEffects / 360.0) * 6.2831854820251465);
    }
    ...
    {
      return Vector4.Lerp(new Vector4(0.647058845f, 0.509803951f, 0.933333337f, 1f), new Vector4(0.8039216f, 0.8039216f, 1f, 1f), (float) (0.10000000149011612 + (double) LiquidRenderer.GetShimmerWave(ref worldPositionX, ref worldPositionY) * 0.40000000596046448));
    }

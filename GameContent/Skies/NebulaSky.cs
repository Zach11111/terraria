...
namespace Terraria.GameContent.Skies
{
  public class NebulaSky : CustomSky
  {
    ...
    {
      if ((double) maxDepth >= 3.4028234663852886E+38 && (double) minDepth < 3.4028234663852886E+38)
      {
        ...
        spriteBatch.Draw(this._bgTexture.Value, new Rectangle(0, Math.Max(0, (int) ((Main.worldSurface * 16.0 - (double) Main.screenPosition.Y - 2400.0) * 0.10000000149011612)), Main.screenWidth, Main.screenHeight), Color.White * Math.Min(1f, (float) (((double) Main.screenPosition.Y - 800.0) / 1000.0) * this._fadeOpacity));
      ...
      {
        Vector2 vector2_4 = new Vector2(1f / this._pillars[a].Depth, 0.9f / this._pillars[a].Depth);
        Vector2 position = (this._pillars[a].Position - vector2_3) * vector2_4 + vector2_3 - Main.screenPosition;
        if (rectangle.Contains((int) position.X, (int) position.Y))
        {
          ...
          {
            float num6 = (float) (1.0 - ((double) num5 + (double) Main.GlobalTimeWrappedHourly * 0.019999999552965164 + Math.Sin((double) a)) % 1.0);
            spriteBatch.Draw(this._rockTextures[index].Value, position + new Vector2((float) (Math.Sin((double) num5 * 1582.0) * ((double) num4 * 0.5) + (double) num4 * 0.5), num6 * 2000f), new Rectangle?(), Color.White * num6 * num3 * this._fadeOpacity, num6 * 20f, new Vector2((float) (this._rockTextures[index].Width() >> 1), (float) (this._rockTextures[index].Height() >> 1)), 0.9f, SpriteEffects.None, 0.0f);
            index = (index + 1) % this._rockTextures.Length;
          }
        }
      }
    }
    ...
    {
      return (float) ((1.0 - (double) this._fadeOpacity) * 0.30000001192092896 + 0.699999988079071);
    }

    ...
    public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
    {
      ...
      for (int index = num1; index < num2; ++index)
      {
        ...
        if (rectangle.Contains((int) position.X, (int) position.Y))
        {
          ...
          float num5 = (float) (Math.Sin((double) this._stars[index].AlphaFrequency * (double) Main.GlobalTimeWrappedHourly * 5.0 + (double) this._stars[index].SinOffset) * 0.10000000149011612 - 0.10000000149011612);
          float num6 = MathHelper.Clamp(num4, 0.0f, 1f);
          Texture2D texture = this._starTextures[this._stars[index].TextureIndex].Value;
          spriteBatch.Draw(texture, position, new Rectangle?(), Color.White * num3 * num6 * 0.8f * (1f - num5) * this._fadeOpacity, 0.0f, new Vector2((float) (texture.Width >> 1), (float) (texture.Height >> 1)), (float) (((double) vector2_4.X * 0.5 + 0.5) * ((double) num6 * 0.30000001192092896 + 0.699999988079071)), SpriteEffects.None, 0.0f);
        }
      }
    }

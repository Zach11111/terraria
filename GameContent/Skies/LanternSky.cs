...
namespace Terraria.GameContent.Skies
{
  public class LanternSky : CustomSky
  {
    ...
    {
      this._lanterns[i].Depth = (float) ((1.0 - (double) i / (double) this._lanterns.Length) * 4.4000000953674316 + 1.6000000238418579);
    ...
    public override void Update(GameTime gameTime)
    {
      ...
      for (int i = 0; i < this._lanterns.Length; ++i)
      {
        if (this._lanterns[i].Active)
        {
          ...
          this._lanterns[i].Position.X += (float) ((0.10000000149011612 + (double) num1) * (3.0 - (double) this._lanterns[i].Speed) * 0.5 * ((double) i / (double) this._lanterns.Length + 1.5) / 2.5);
    ...
    private void DrawLantern(
      ...
      float alpha)
    {
      float y = ((float) ((double) Main.GlobalTimeWrappedHourly % 6.0 / 6.0 * 6.2831854820251465)).ToRotationVector2().Y;
      float num1 = (float) ((double) y * 0.20000000298023224 + 0.800000011920929);
      Color color = new Color((int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue, 0) * this._opacity * alpha * num1 * 0.4f;
      for (float num2 = 0.0f; (double) num2 < 1.0; num2 += 0.333333343f)
      {
        Vector2 vector2 = new Vector2(0.0f, 2f).RotatedBy(6.2831854820251465 * (double) num2 + (double) lantern.Rotation) * y;
    ...
    private struct Lantern
    {
      ...
      {
        get => this.Speed * ((float) this.TimeUntilFloat / (float) this.TimeUntilFloatMax);
      }

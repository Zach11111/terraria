...
namespace Terraria.GameContent.Golf
{
  public static class GolfHelper
  {
    ...
    {
      return (double) Math.Abs(radius.X) < 9.9999997473787516E-05 || (double) Math.Abs(radius.Y) < 9.9999997473787516E-05 ? Vector2.Zero : Vector2.Normalize(vector / radius) * radius;
    }
    ...
    {
      return (int) golfBall.localAI[1] == 0 || (double) Vector2.Distance(golfBall.position, golfBall.oldPos[golfBall.oldPos.Length - 1]) < 1.0;
    }

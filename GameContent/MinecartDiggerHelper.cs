...
namespace Terraria.GameContent
{
  public class MinecartDiggerHelper
  {
    ...
    {
      Item minecartTrackItem = (Item) null;
      for (int index = 0; index < 58; ++index)
      {
        if (player.selectedItem != index || player.itemAnimation <= 0 && player.reuseDelay <= 0 && player.itemTime <= 0)
        {
          ...
          if (obj.type == 2340 && obj.stack > 0)
          {
            minecartTrackItem = obj;
            break;
          }
        }
      }
      return minecartTrackItem;
    }
    ...
    private void PlaceATrack(int x, int y)
    {
      int num1 = 314;
      int num2 = 0;
      if (!WorldGen.PlaceTile(x, y, num1, plr: Main.myPlayer, style: num2))
        return;
      NetMessage.SendData(17, number: 1, number2: (float) x, number3: (float) y, number4: (float) num1, number5: num2);
    }

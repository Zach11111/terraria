...
namespace Terraria.GameContent.Events
{
  public class DD2Event
  {
    ...
    {
      get => DD2Event.DownedInvasionT1 || DD2Event.DownedInvasionT2 || DD2Event.DownedInvasionT3;
    }
    ...
    public static bool EnemySpawningIsOnHold => DD2Event._timeLeftUntilSpawningBegins != 0;
    ...
    {
      WorldGen.BroadcastText(NetworkText.FromKey("DungeonDefenders2.InvasionLose"), DD2Event.INFO_FAILURE_INVASION_COLOR);
    }
    ...
    public static void CheckProgress(int slainMonsterID)
    {
      ...
      if (flag)
      {
        ...
        int num4 = 1;
        if (Main.netMode != 1)
          Main.ReportInvasionProgress(num3, num4, 3, num1);
        if (Main.netMode != 2)
          return;
        NetMessage.SendData(78, number: num3, number2: (float) num4, number3: 3f, number4: (float) num1);
      }
      else
      {
        if (Main.netMode != 1)
    ...
    public static void ClearAllDD2EnergyCrystalsInChests()
    {
      ...
      for (int number = 0; number < 8000; ++number)
      {
        ...
        if (chest != null && currentlyOpenChests.Contains(number))
        {
          ...
          {
            if (chest.item[number2].type == 3822 && chest.item[number2].stack > 0)
            {
              chest.item[number2].TurnToAir();
              if (Main.netMode != 0)
                NetMessage.SendData(32, number: number, number2: (float) number2);
            }
          }
        }
      }
    ...
    {
      return (IEntitySource) new EntitySource_OldOnesArmy();
    }
  }
}

...
namespace Terraria.GameContent.Events
{
  public class Sandstorm
  {
    ...
    {
      return (double) Math.Abs(Main.windSpeedCurrent) >= 0.60000002384185791;
    }
    ...
    public static void EmitDust()
    {
      ...
      if ((double) amount < 0.0099999997764825821)
        return;
      ...
      float num8 = (float) ((double) num6 * ((double) Main.gfxQuality * 0.5 + 0.5) + (double) num6 * 0.10000000149011612) - (float) Dust.SandStormCount;
      if ((double) num8 <= 0.0)
        return;
      ...
      int maxValue2 = (int) MathHelper.Lerp(1f, 10f, (float) (((double) num4 - 0.76999998092651367) / 0.23000001907348633));
      for (int index1 = 0; (double) index1 < (double) num7; ++index1)
      {
        if (Main.rand.Next(num5 / 4) == 0)
        {
          ...
          if ((double) Position.X < 0.0 || (double) Position.X > (double) Main.screenWidth)
            Position.Y += (float) ((double) Main.rand.NextFloat() * (double) screenHeight * 0.89999997615814209);
          Position += vector2;
          ...
          if (WorldGen.InWorld(x1, y1, 10) && Main.tile[x1, y1] != null && Main.tile[x1, y1].wall == (ushort) 0)
          {
            for (int index2 = 0; index2 < 1; ++index2)
            {
              ...
              dust.velocity.Y = (float) (2.0 + (double) Main.rand.NextFloat() * 0.20000000298023224);

...
namespace Terraria.GameContent.Drawing
{
  public class TileDrawing
  {
    ...
    public void Update()
    {
      ...
      this._vineWindCounter += 1.0 / 120.0 + 1.0 / 120.0 * lerpValue * 0.40000000596046448;
      ...
      int tileY)
    {
      ...
      {
        short index1 = Main.tileGlowMask[(int) drawData.tileCache.type];
        if (TextureAssets.GlowMask.IndexInRange<Asset<Texture2D>>((int) index1))
        ...
        switch (drawData.tileCache.type)
        {
          ...
          case 209:
            ...
            break;
          case 350:
            color = new Color(new Vector4((float) (-Math.Cos((int) (num3 / 6.283) % 3 == 1 ? num3 : 0.0) * 0.2 + 0.2)));
            break;
          case 370:
          case 390:
    ...
    {
      Texture2D tileDrawTexture = TextureAssets.Tile[(int) tile.type].Value;
      int tileStyle = 0;
      ...
      if (requestIfNotReady != null)
        tileDrawTexture = requestIfNotReady;
      return tileDrawTexture;
    }
      ...
      if (drawData.tileCache.slope() > (byte) 0)
      {
        ...
        else
        {
          ...
          for (int index = 0; index < 8; ++index)
          {
            ...
            Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2((float) x, (float) (index * width + num2)), new Rectangle?(new Rectangle((int) drawData.tileFrameX + drawData.addFrX + x, (int) drawData.tileFrameY + drawData.addFrY + num3, width, height)), drawData.finalColor, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
          }
          int y = num1 > 2 ? 0 : 14;
          Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(0.0f, (float) y), new Rectangle?(new Rectangle((int) drawData.tileFrameX + drawData.addFrX, (int) drawData.tileFrameY + drawData.addFrY + y, 16, 2)), drawData.finalColor, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
        }
      }
      else if (!TileID.Sets.Platforms[(int) drawData.typeCache] && !TileID.Sets.IgnoresNearbyHalfbricksWhenDrawn[(int) drawData.typeCache] && this._tileSolid[(int) drawData.typeCache] && !TileID.Sets.NotReallySolid[(int) drawData.typeCache] && !drawData.tileCache.halfBrick() && (Main.tile[tileX - 1, tileY].halfBrick() || Main.tile[tileX + 1, tileY].halfBrick()))
      {
        ...
        else if (Main.tile[tileX - 1, tileY].halfBrick())
        {
          ...
          if (TileID.Sets.AllBlocksWithSmoothBordersToResolveHalfBlockIssue[(int) drawData.typeCache])
            num = 2;
          Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2(0.0f, 8f), new Rectangle?(new Rectangle((int) drawData.tileFrameX + drawData.addFrX, drawData.addFrY + (int) drawData.tileFrameY + 8, drawData.tileWidth, 8)), drawData.finalColor, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
          Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition + new Vector2((float) num, 0.0f), new Rectangle?(new Rectangle((int) drawData.tileFrameX + num + drawData.addFrX, drawData.addFrY + (int) drawData.tileFrameY, drawData.tileWidth - num, drawData.tileHeight)), drawData.finalColor, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
          Main.spriteBatch.Draw(drawData.drawTexture, normalTilePosition, new Rectangle?(new Rectangle(144 + drawData.addFrX, drawData.addFrY, num, 8)), drawData.finalColor, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
          if (num != 2)
            return;
        ...
        if ((double) graveyardVisualIntensity > 0.0)
        {
          ...
          for (int index = 0; index < tileFlameData.flameCount; ++index)
          {
            ...
            for (float num2 = 0.0f; (double) num2 < 1.0; num2 += 0.25f)
              Main.spriteBatch.Draw(tileFlameData.flameTexture, position + new Vector2(x, y) + Vector2.UnitX.RotatedBy((double) num2 * 6.2831854820251465) * 2f, new Rectangle?(rectangle), color, 0.0f, TileDrawing._zero, 1f, drawData.tileSpriteEffect, 0.0f);
...
label_323:
    ...
    private void EmitLivingTreeLeaf_Below(int x, int y, int leafGoreType)
    {
      ...
      if ((double) windForVisuals < -0.20000000298023224 && (WorldGen.SolidTile(Main.tile[x - 1, y + 1]) || WorldGen.SolidTile(Main.tile[x - 2, y + 1])) || (double) windForVisuals > 0.20000000298023224 && (WorldGen.SolidTile(Main.tile[x + 1, y + 1]) || WorldGen.SolidTile(Main.tile[x + 2, y + 1])))
        return;
    ...
    {
      int num1 = 0;
      if ((double) Main.WindForVisuals > 0.20000000298023224)
        num1 = 1;
      else if ((double) Main.WindForVisuals < -0.20000000298023224)
        num1 = -1;
    ...
    private void UpdateLeafFrequency()
    {
      ...
      this._leafFrequency = (double) num > 0.10000000149011612 ? ((double) num > 0.20000000298023224 ? ((double) num > 0.30000001192092896 ? ((double) num > 0.40000000596046448 ? ((double) num > 0.5 ? ((double) num > 0.60000002384185791 ? ((double) num > 0.699999988079071 ? ((double) num > 0.800000011920929 ? ((double) num > 0.89999997615814209 ? ((double) num > 1.0 ? ((double) num > 1.1000000238418579 ? 10 : 20) : 30) : 40) : 50) : 75) : 130) : 200) : 300) : 450) : 1000) : 2000;
    ...
    private void EmitTreeLeaves(int tilePosX, int tilePosY, int grassPosX, int grassPosY)
    {
      ...
      if (flag2)
      {
        int num1 = tilePosX - grassPosX;
        vector2.X += (float) (num1 * 12);
        int num2 = 0;
      ...
      if (WorldGen.SolidTile(vector2.ToTileCoordinates()))
        return;
      Gore.NewGoreDirect(vector2, Utils.RandomVector2(Main.rand, (float) -max, (float) max), passStyle, (float) (0.699999988079071 + (double) Main.rand.NextFloat() * 0.60000002384185791)).Frame.CurrentColumn = Main.tile[tilePosX, tilePosY].color();
    }
    ...
    private void DrawSpecialTilesLegacy(Vector2 screenPosition, Vector2 offSet)
    {
      for (int index1 = 0; index1 < this._specialTilesCount; ++index1)
      {
        ...
        if (type1 == (ushort) 620)
        {
          ...
          {
            Color color = (new Color((int) sbyte.MaxValue, (int) sbyte.MaxValue, (int) sbyte.MaxValue, 0).MultiplyRGBA(Main.hslToRgb((float) (((double) Main.GlobalTimeWrappedHourly + (double) index5 / (double) num11) % 1.0), 1f, 0.5f)) * (float) (1.0 - (double) num12 * 0.5)) with
            {
              A = 0
            };
            int num13 = 2;
            Vector2 position2 = position1 + ((float) ((double) index5 / (double) num11 * 6.2831854820251465)).ToRotationVector2() * (float) ((double) num13 * (double) num12 + 2.0);
            ...
            if (type == (ushort) 323)
            {
              if (frameX >= (short) 88)
              {
                if (frameX <= (short) 132)
                {
                  ...
                  Vector2 position = new Vector2((float) (x * 16 - (int) unscaledPosition.X - num14 + frameY2 + width / 2), (float) (y1 * 16 - (int) unscaledPosition.Y + 16 + num15)) + zero;
                  float num16 = 0.0f;
                  if (!flag1)
                    num16 = this.GetWindCycle(x, y1, this._treeWindCounter);
                  position.X += num16 * 2f;
      ...
      for (int index2 = 0; index2 < num1; ++index2)
      {
        ...
        if (tile != null && tile.active() && this.IsVisible(tile))
        {
          ...
          if (!this.InAPlaceWithWind(x, y, 1, 1))
            num2 = 0.0f;
          float num3 = num2 + this.GetWindGridPush(x, y, 20, 0.35f);
          position.X += num3 * 1f;
    ...
    private void DrawMasterTrophies()
    {
      ...
      for (int index2 = 0; index2 < num1; ++index2)
      {
        ...
        if (tile != null && tile.active())
        {
          ...
          float num3 = (float) Math.Sin((double) Main.GlobalTimeWrappedHourly * 6.2831854820251465 / 5.0);
          ...
          float num4 = (float) (Math.Sin((double) Main.GlobalTimeWrappedHourly * 6.2831854820251465 / 2.0) * 0.30000001192092896 + 0.699999988079071);
          Color color2 = color1 with { A = (byte) 0 } * 0.1f * num4;
          for (float num5 = 0.0f; (double) num5 < 1.0; num5 += 0.166666672f)
            Main.spriteBatch.Draw(texture2D, vector2_2 - Main.screenPosition + (6.28318548f * num5).ToRotationVector2() * (float) (6.0 + (double) num3 * 2.0), new Rectangle?(r), color2, 0.0f, origin, 1f, effects, 0.0f);
        }
      }
    }
    ...
    private void DrawTeleportationPylons()
    {
      ...
      for (int index2 = 0; index2 < num1; ++index2)
      {
        ...
        if (tile != null && tile.active())
        {
          ...
          Color color2 = new Color((int) byte.MaxValue, (int) byte.MaxValue, (int) byte.MaxValue, 0) * 0.1f * (float) (Math.Sin((double) Main.GlobalTimeWrappedHourly * 6.2831854820251465 / 1.0) * 0.20000000298023224 + 0.800000011920929);
          for (float num4 = 0.0f; (double) num4 < 1.0; num4 += 0.166666672f)
            Main.spriteBatch.Draw(texture2D, center - Main.screenPosition + (6.28318548f * num4).ToRotationVector2() * (float) (6.0 + (double) num3 * 2.0), new Rectangle?(r), color2, 0.0f, origin, 1f, SpriteEffects.None, 0.0f);
          int num5 = 0;
          bool actuallySelected;
    ...
    private float GetHighestWindGridPushComplex(
      ...
      bool swapLoopDir)
    {
      float windGridPushComplex1 = 0.0f;
      ...
      for (int index1 = 0; index1 < 1; ++index1)
      {
        for (int index2 = 0; index2 < sizeY; ++index2)
        {
          ...
          {
            windGridPushComplex1 = windGridPushComplex2;
            num = windTimeLeft;
          }
        }
      }
      return windGridPushComplex1;
    }

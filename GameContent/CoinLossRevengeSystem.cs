...
namespace Terraria.GameContent
{
  public class CoinLossRevengeSystem
  {
    ...
    public void AddMarkerFromReader(BinaryReader reader)
    {
      ...
      int npcNetId = num2;
      double npcHPPercent = (double) num3;
      int npcType = num4;
      int npcAiStyle = num5;
      int coinValue = num6;
      double baseValue = (double) num7;
      int num8 = flag ? 1 : 0;
      int gameTime = this._gameTime;
      int uniqueID = num1;
      this.AddMarker(new CoinLossRevengeSystem.RevengeMarker(coords, npcNetId, (float) npcHPPercent, npcType, npcAiStyle, coinValue, (float) baseValue, num8 != 0, gameTime, uniqueID));
    }
    ...
    public void CheckRespawns()
    {
      ...
      lock (this._markersLock)
      {
        List<CoinLossRevengeSystem.RevengeMarker> second = new List<CoinLossRevengeSystem.RevengeMarker>();
        ...
        this._markers = this._markers.Except<CoinLossRevengeSystem.RevengeMarker>((IEnumerable<CoinLossRevengeSystem.RevengeMarker>) second).ToList<CoinLossRevengeSystem.RevengeMarker>();
      }
    }

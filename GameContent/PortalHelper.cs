...
namespace Terraria.GameContent
{
  public class PortalHelper
  {
                  ...
                  if (!(Collision.TileCollision(newPos - Velocity3, Velocity3, width, height, true, true, gravDir) != Velocity3))
                  {
                    ...
                    if (!(Collision.TileCollision(newPos - Velocity4, Velocity4, width, height, true, true, gravDir) != Velocity4))
                    {
                      ...
                      if (ent.velocity == Vector2.Zero)
                        ent.velocity = (projectile1.ai[0] - 1.57079637f).ToRotationVector2() * num2;
                      ...
                      if (bonusY == -gravDir && Math.Sign(ent.velocity.Y) != -gravDir || (double) Math.Abs(ent.velocity.Y) < 0.10000000149011612)
                        ent.velocity.Y = (float) -gravDir * 0.1f;
                      int num3 = (int) ((double) (projectile2.owner * 2) + (double) projectile2.ai[1]);
                      int num4 = num3 + (num3 % 2 == 0 ? 1 : -1);
                      switch (ent)
                      {
                        case Player _:
                          Player player = (Player) ent;
                          player.lastPortalColorIndex = num4;
                          player.Teleport(newPos, 4, num3);
                          if (Main.netMode == 1)
                          {
                            NetMessage.SendData(96, number: player.whoAmI, number2: newPos.X, number3: newPos.Y, number4: (float) num3);
                        ...
                        case NPC _:
                          NPC npc = (NPC) ent;
                          npc.lastPortalColorIndex = num4;
                          npc.Teleport(newPos, 4, num3);
                          if (Main.netMode == 2)
                          {
                            NetMessage.SendData(100, number: npc.whoAmI, number2: newPos.X, number3: newPos.Y, number4: (float) num3);
                            NetMessage.SendData(23, number: npc.whoAmI);
                          }
                          PortalHelper.PortalCooldownForPlayers[index1] = 10;
                          if (bonusY != -1 || (double) ent.velocity.Y <= -3.0)
                            return;
                          ent.velocity.Y = -3f;
                          return;
                        default:
    ...
    private static bool DoesTileTypeSupportPortals(ushort tileType) => tileType != (ushort) 496;
    ...
    private static Vector2 FindCollision(Vector2 startPosition, Vector2 stopPosition)
    {
      int lastX = 0;
      int lastY = 0;
      Utils.PlotLine(startPosition.ToTileCoordinates(), stopPosition.ToTileCoordinates(), (Utils.TileActionAttempt) ((x, y) =>
      {
        lastX = x;
        lastY = y;
        ...
        if (projectile.active && projectile.type == 602)
        {
          ...
          if (Collision.CheckLinevLine(start1, end1, start2, end2).Length != 0)
          {
            if (projectile.owner != Main.myPlayer && Main.netMode != 2)
              NetMessage.SendData(95, number: projectile.owner, number2: (float) (int) projectile.ai[1]);
            projectile.Kill();
          }
        }
      }
    ...
    public static Color GetPortalColor(int player, int portal)
    {
      ...
      if (Main.netMode == 0)
      {
        portalColor = portal != 0 ? Main.hslToRgb(0.52f, 1f, 0.6f) : Main.hslToRgb(0.12f, 1f, 0.5f);
      }
      else
      {
        float num = 0.08f;
        portalColor = Main.hslToRgb((float) ((0.5 + (double) player * ((double) num * 2.0) + (double) portal * (double) num) % 1.0), 1f, 0.5f);
      }
      portalColor.A = (byte) 66;
      return portalColor;
    }
    ...
    private static Vector2 GetPortalOutingPoint(
      ...
      out int bonusY)
    {
      int num = (int) Math.Round((double) MathHelper.WrapAngle(portalAngle) / 0.78539818525314331);
      switch (num)
      {
        case -3:
        case 3:
          ...
          for (int x = sectionX - fluff; x < sectionX + fluff + 1; ++x)
          {
            for (int y = sectionY - fluff; y < sectionY + fluff + 1; ++y)
            {
              if (x >= 0 && x < Main.maxSectionsX && y >= 0 && y < Main.maxSectionsY && !Netplay.Clients[plr].TileSections[x, y] && !dontInclude.Contains(new Point(x, y)))
                portalSections.Add(new Point(x, y));
            }
          }
        }
      }
    ...
    public static bool SupportedTilesAreFine(Vector2 portalCenter, float portalAngle)
    {
      ...
      int num1 = (int) Math.Round((double) MathHelper.WrapAngle(portalAngle) / 0.78539818525314331);
      int num2;
      int num3;
      switch (num1)
      {

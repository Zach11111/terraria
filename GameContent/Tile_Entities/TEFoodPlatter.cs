...
namespace Terraria.GameContent.Tile_Entities
{
  public class TEFoodPlatter : TileEntity, IFixLoadedData
  {
    ...
    {
      NetMessage.SendData(86, number: TEFoodPlatter.Place(x, y), number2: (float) x, number3: (float) y);
    }
    ...
    {
      if (Main.netMode != 1)
        Item.NewItem((IEntitySource) new EntitySource_TileBreak((int) this.Position.X, (int) this.Position.Y), (int) this.Position.X * 16, (int) this.Position.Y * 16, 16, 16, this.item.netID, pfix: (int) this.item.prefix);
      this.item = new Item();
    }
    ...
    public static void TryPlacing(int x, int y, int netid, int prefix, int stack)
    {
      ...
      if (key == -1)
      {
        int number = Item.NewItem((IEntitySource) new EntitySource_TileBreak(x, y), x * 16, y * 16, 16, 16, 1);
    ...
    public static void OnPlayerInteraction(Player player, int clickX, int clickY)
    {
      ...
      else
      {
        int num1 = clickX;
        int num2 = clickY;
        int key = TEFoodPlatter.Find(num1, num2);
        ...
        if (Main.netMode != 1)
          return;
        NetMessage.SendData(17, number2: (float) num1, number3: (float) num2, number4: 1f);
      }
    }

...
namespace Terraria.GameContent.Tile_Entities
{
  public class TEDisplayDoll : TileEntity, IFixLoadedData
  {
    ...
    {
      NetMessage.SendData(86, number: TEDisplayDoll.Place(x, y), number2: (float) x, number3: (float) y);
    }
    ...
    {
      return this.Position.X.ToString() + "x  " + (object) this.Position.Y + "y item: " + (object) this._items[0] + " " + (object) this._items[1] + " " + (object) this._items[2];
    }
    ...
    private bool TryFitting(Item[] inv, int context = 0, int slot = 0, bool justCheck = false)
    {
      ...
      if (obj.legSlot > 0)
        number3 = 2;
      if (obj.accessory)
        number3 = TEDisplayDoll.accessoryTargetSlot;
      if (number3 == -1)
        return false;
      if (justCheck)
        return true;
      if (obj.accessory)
      {
        ...
        for (int index = 3; index < 8; ++index)
        {
          ...
          {
            number3 = index;
            TEDisplayDoll.accessoryTargetSlot = index;
            break;
          }
        }
      ...
      if (Main.netMode == 1)
        NetMessage.SendData(121, number: Main.myPlayer, number2: (float) this.ID, number3: (float) number3);
      return true;
    }
    ...
    {
      return Main.tile[x, y].active() && Main.tile[x, y].type == (ushort) 470 && Main.tile[x, y].frameY == (short) 0 && (int) Main.tile[x, y].frameX % 36 == 0;
    }

  ...
  public class TEItemFrame : TileEntity, IFixLoadedData
  {
    ...
    public static void OnPlayerInteraction(Player player, int clickX, int clickY)
    {
      ...
      else
      {
        int index1 = clickX;
        int index2 = clickY;
        if ((int) Main.tile[index1, index2].frameX % 36 != 0)
          --index1;
        if ((int) Main.tile[index1, index2].frameY % 36 != 0)
          --index2;
        int key = TEItemFrame.Find(index1, index2);
        ...
        if (Main.netMode != 1)
          return;
        NetMessage.SendData(17, number2: (float) index1, number3: (float) index2, number4: 1f);
      }
    }

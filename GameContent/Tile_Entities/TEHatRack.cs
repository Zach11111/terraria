...
namespace Terraria.GameContent.Tile_Entities
{
  public class TEHatRack : TileEntity, IFixLoadedData
  {
    ...
    {
      NetMessage.SendData(86, number: TEHatRack.Place(x, y), number2: (float) x, number3: (float) y);
    }
    ...
    {
      return this.Position.X.ToString() + "x  " + (object) this.Position.Y + "y item: " + (object) this._items[0] + " " + (object) this._items[1];
    }
    ...
    public static void Framing_CheckTile(int callX, int callY)
    {
      ...
      if (!flag)
        return;
      TEHatRack.Kill(num3, num4);
      Item.NewItem((IEntitySource) new EntitySource_TileBreak(num3, num4), num3 * 16, num4 * 16, 48, 64, 3977);
    ...
    private bool TryFitting(Item[] inv, int context = 0, int slot = 0, bool justCheck = false)
    {
      ...
      if (justCheck)
        return true;
      int number3 = TEHatRack.hatTargetSlot;
      ...
      for (int index = 0; index < 2; ++index)
      {
        ...
        {
          number3 = index;
          TEHatRack.hatTargetSlot = index + 1;
          break;
        }
      }
      for (int index = 0; index < 2; ++index)
      ...
      if (Main.netMode == 1)
        NetMessage.SendData(124, number: Main.myPlayer, number2: (float) this.ID, number3: (float) number3);
      return true;
    }
    ...
    {
      return Main.tile[x, y].active() && Main.tile[x, y].type == (ushort) 475 && Main.tile[x, y].frameY == (short) 0 && (int) Main.tile[x, y].frameX % 54 == 0;
    }

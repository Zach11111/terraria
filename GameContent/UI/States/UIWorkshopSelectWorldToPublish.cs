...
// Type: Terraria.GameContent.UI.States.UIWorkshopSelectWorldToPublish
...
namespace Terraria.GameContent.UI.States
{
  public class UIWorkshopSelectWorldToPublish : UIState, IHaveBackButtonCommand
  {
    ...
    public override void OnInitialize()
    {
      ...
      UITextPanel<LocalizedText> element3 = new UITextPanel<LocalizedText>(Language.GetText("UI.WorkshopSelectWorldToPublishMenuTitle"), 0.8f, true);
    ...
    private void PopulateEntries()
    {
      ...
      IOrderedEnumerable<WorldFileData> orderedEnumerable = new List<WorldFileData>((IEnumerable<WorldFileData>) Main.WorldList).OrderByDescending<WorldFileData, bool>((Func<WorldFileData, bool>) (x => x.IsFavorite)).ThenBy<WorldFileData, string>((Func<WorldFileData, string>) (x => x.Name)).ThenBy<WorldFileData, string>((Func<WorldFileData, string>) (x => x.GetFileName()));
      this._entryList.Clear();
      int num = 0;
      foreach (WorldFileData data in (IEnumerable<WorldFileData>) orderedEnumerable)
        this._entryList.Add((UIElement) new UIWorkshopPublishWorldListItem((UIState) this, data, num++));
    }

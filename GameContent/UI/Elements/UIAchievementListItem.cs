...
namespace Terraria.GameContent.UI.Elements
{
  public class UIAchievementListItem : UIPanel
  {
    ...
    protected override void DrawSelf(SpriteBatch spriteBatch)
    {
      ...
      int x = this._large.ToInt() * 6;
      Vector2 vector2_1 = new Vector2((float) x, 0.0f);
      ...
      if ((!(trackerValues.Item1 == 0M) || !(trackerValues.Item2 == 0M)) && this._locked)
        flag = true;
      float num1 = (float) ((double) innerDimensions.Width - (double) dimensions.Width + 1.0) - (float) (x * 2);
      Vector2 baseScale1 = new Vector2(0.85f);
      Vector2 baseScale2 = new Vector2(0.92f);
      string wrappedText = FontAssets.ItemStack.Value.CreateWrappedText(this._achievement.Description.Value, (float) (((double) num1 - 20.0) * (1.0 / (double) baseScale2.X)), Language.ActiveCulture.CultureInfo);
      Vector2 stringSize1 = ChatManager.GetStringSize(FontAssets.ItemStack.Value, wrappedText, baseScale2, num1);
      if (!this._large)
        stringSize1 = ChatManager.GetStringSize(FontAssets.ItemStack.Value, this._achievement.Description.Value, baseScale2, num1);
      float num2 = (float) (38.0 + (this._large ? 20.0 : 0.0));
      if ((double) stringSize1.Y > (double) num2)
        baseScale2.Y *= num2 / stringSize1.Y;
      ...
      this.DrawPanelTop(spriteBatch, position1, num1, color1);
      ...
      position1.X += 4f;
      position1.X += 17f;
      ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.Value, this._achievement.FriendlyName.Value, position1, baseColor1, 0.0f, Vector2.Zero, baseScale1, num1);
      position1.X -= 17f;
      Vector2 position2 = vector2_2 + Vector2.UnitY * 27f + vector2_1;
      this.DrawPanelBottom(spriteBatch, position2, num1, color1);
      position2.X += 8f;
      position2.Y += 4f;

...
namespace Terraria.GameContent
{
  public struct PlayerSleepingHelper
  {
    ...
    {
      return NPC.AnyDanger(true) || Main.bloodMoon && !Main.dayTime || Main.eclipse && Main.dayTime || player.itemAnimation > 0;
    }
    ...
    public void SetIsSleepingAndAdjustPlayerRotation(Player player, bool state)
    {
      ...
      if (state)
      {
        player.fullRotation = 1.57079637f * (float) -player.direction;
      }
      else
      {

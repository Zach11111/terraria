  ...
  public class ThinIceBiome : MicroBiome
  {
    public override bool Place(Point origin, StructureMap structures)
    {
      ...
      for (int radius = GenBase._random.Next(10, 15); radius > 5; --radius)
      {
        ...
        WorldUtils.Gen(new Point(origin.X + num5, origin.Y + num4), (GenShape) new Shapes.Circle(radius), Actions.Chain((GenAction) new Modifiers.Blotches(4), (GenAction) new Modifiers.HasLiquid(), (GenAction) new Modifiers.SkipTiles(new ushort[4]
        {
          (ushort) 21,
          (ushort) 467,
          (ushort) 226,
          (ushort) 237
        }), (GenAction) new Actions.SetTile((ushort) 162, true), (GenAction) new Actions.SetLiquid(value: (byte) 0)));
        num4 += radius - 2;
      }

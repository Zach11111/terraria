...
namespace Terraria.Social.Steam
{
  public class NetServerSocialModule : NetSocialModule
  {
    ...
    private void BroadcastConnectedUsers()
    {
      ...
      foreach (KeyValuePair<CSteamID, NetSocialModule.ConnectionState> connectionState in this._connectionStateMap)
      {
        if (connectionState.Value == NetSocialModule.ConnectionState.Connected)
          ulongList.Add(connectionState.Key.m_SteamID);
      }
    ...
    public override ulong GetLobbyId() => this._lobby.Id.m_SteamID;
    ...
    private bool OnPacketRead(byte[] data, int length, CSteamID userId)
    {
      ...
      if (length < 3 || ((int) data[1] << 8 | (int) data[0]) != length || data[2] != (byte) 93)
        return false;
      byte[] destinationArray = new byte[data.Length - 3];
      Array.Copy((Array) data, 3, (Array) destinationArray, 0, destinationArray.Length);
      switch ((int) SteamUser.BeginAuthSession(destinationArray, destinationArray.Length, userId))
      {
        case 0:
    ...
    private void OnP2PSessionRequest(P2PSessionRequest_t result)
    {
      ...
      else
      {
        ...
        do
          ;
        while (SteamNetworking.GetP2PSessionState(steamIdRemote, ref p2PsessionStateT) && p2PsessionStateT.m_bConnecting == (byte) 1);
        if (p2PsessionStateT.m_bConnectionActive == (byte) 0)

  ...
  public class NetClientSocialModule : NetSocialModule
  {
    ...
    private void OnLobbyJoinRequest(GameLobbyJoinRequested_t result)
    {
      ...
      Main.QueueMainThreadAction((Action) (() => Main.OpenPlayerSelect((Main.OnPlayerSelected) (playerData =>
      {
        ...
        // ISSUE: method pointer
        this._lobby.Join(result.m_steamIDLobby, new CallResult<LobbyEnter_t>.APIDispatchDelegate((object) this, __methodptr(OnLobbyEntered)));
      }))));
    }
    ...
    private void OnLobbyEntered(LobbyEnter_t result, bool failure)
    {
      ...
      while (SteamNetworking.GetP2PSessionState(this._lobby.Owner, ref p2PsessionStateT) && p2PsessionStateT.m_bConnectionActive != (byte) 1)
      {
    ...
    private void SendAuthTicket(CSteamID address)
    {
      ...
      if (HAuthTicket.op_Equality(this._authTicket, HAuthTicket.Invalid))
    ...
    {
      if (HAuthTicket.op_Inequality(this._authTicket, HAuthTicket.Invalid))

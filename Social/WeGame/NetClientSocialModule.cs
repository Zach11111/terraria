...
namespace Terraria.Social.WeGame
{
  public class NetClientSocialModule : NetSocialModule
  {
    ...
    {
      rail_api.RailFactory().RailFriends().AsyncSetMyMetadata(new List<RailKeyValue>()
      {
        new RailKeyValue() { key = key, value = value }
      }, "");
    }
    ...
    {
      rail_api.RailFactory().RailFriends()?.AsyncReportPlayedWithUserList(new List<RailUserPlayedWith>()
      {
        new RailUserPlayedWith() { rail_id = rail_id }
      }, "");
    }

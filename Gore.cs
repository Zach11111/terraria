...
namespace Terraria
{
  public class Gore
  {
    ...
    public byte numFrames
    {
      ...
      set
      {
        this.Frame = new SpriteFrame(this.Frame.ColumnCount, value)
        {
    ...
    {
      float waterLine = this.position.Y + this.Height;
        ...
        default:
          ...
          else if (this.type == 1201)
          {
            ...
            else
            {
              ...
              if ((double) num > 1.0)
                this.velocity *= 0.1f;
              else if ((double) num > 0.10000000149011612)
                this.velocity *= 0.98f;
            }
          }
          ...
          {
            if (++this.frameCounter >= (byte) 8 && (double) this.velocity.Y > 0.20000000298023224)
            {
              this.frameCounter = (byte) 0;
          ...
          else if (GoreID.Sets.SpecialAI[this.type] != 1 && GoreID.Sets.SpecialAI[this.type] != 2)
          {
            ...
            else if (this.type == 1218)
            {
              if (this.timeLeft > 8)
                this.timeLeft = 8;
              this.velocity.X *= 0.95f;
              if ((double) Math.Abs(this.velocity.X) <= 0.10000000149011612)
          ...
          if (GoreID.Sets.SpecialAI[this.type] == 2)
          {
            ...
            if (this.alpha > (int) byte.MaxValue)
              this.timeLeft = 0;
            this.velocity.X = (float) (((double) this.velocity.X * 50.0 + (double) Main.WindForVisuals * 2.0 + (double) Main.rand.Next(-10, 11) * 0.10000000149011612) / 51.0);
            float num4 = 0.0f;
            if ((double) this.velocity.X < 0.0)
              num4 = this.velocity.X * 0.2f;
            this.velocity.Y = (float) (((double) this.velocity.Y * 50.0 - 0.34999999403953552 + (double) num4 + (double) Main.rand.Next(-10, 11) * 0.20000000298023224) / 51.0);
            ...
            else
            {
              ...
              if (TextureAssets.Gore[this.type].IsLoaded && (double) num5 != -1.0)
              {
                ...
                float Width = (float) TextureAssets.Gore[this.type].Width() * this.scale;
                float Height = (float) TextureAssets.Gore[this.type].Height() * this.scale;
                int Type = 31;
                for (int index8 = 0; (double) index8 < (double) num6; ++index8)
                {
                  int index9 = Dust.NewDust(new Vector2(x, y), (int) Width, (int) Height, Type);
          ...
          if (this.type >= 411 && this.type <= 430)
          {
            this.alpha = 50;
            this.velocity.X = (float) (((double) this.velocity.X * 50.0 + (double) Main.WindForVisuals * 2.0 + (double) Main.rand.Next(-10, 11) * 0.10000000149011612) / 51.0);
            this.velocity.Y = (float) (((double) this.velocity.Y * 50.0 - 0.25 + (double) Main.rand.Next(-10, 11) * 0.20000000298023224) / 51.0);
            ...
            else
            {
              ...
              if (TextureAssets.Gore[this.type].IsLoaded)
              {
                float num = (float) ((double) TextureAssets.Gore[this.type].Width() * (double) this.scale * 0.800000011920929);
                float x = this.position.X;
                float y = this.position.Y;
                float Width = (float) TextureAssets.Gore[this.type].Width() * this.scale;
                float Height = (float) TextureAssets.Gore[this.type].Height() * this.scale;
                ...
                if (this.type >= 426 && this.type <= 430)
                  Type = 179;
                for (int index10 = 0; (double) index10 < (double) num; ++index10)
                {
                  int index11 = Dust.NewDust(new Vector2(x, y), (int) Width, (int) Height, Type);
          ...
          else if (GoreID.Sets.SpecialAI[this.type] != 3 && GoreID.Sets.SpecialAI[this.type] != 1)
          {
            if (this.type >= 706 && this.type <= 717 || this.type == 943 || this.type == 1147 || this.type >= 1160 && this.type <= 1162)
            {
              ...
              {
                float num7 = 0.6f;
                float num8 = this.frame != (byte) 0 ? (this.frame != (byte) 1 ? (this.frame != (byte) 2 ? (this.frame != (byte) 3 ? (this.frame != (byte) 4 ? (this.frame != (byte) 5 ? (this.frame != (byte) 6 ? (this.frame > (byte) 9 ? (this.frame != (byte) 10 ? (this.frame != (byte) 11 ? (this.frame != (byte) 12 ? (this.frame != (byte) 13 ? (this.frame != (byte) 14 ? 0.0f : num7 * 0.1f) : num7 * 0.2f) : num7 * 0.3f) : num7 * 0.4f) : num7 * 0.5f) : num7 * 0.5f) : num7 * 0.2f) : num7 * 0.4f) : num7 * 0.5f) : num7 * 0.4f) : num7 * 0.3f) : num7 * 0.2f) : num7 * 0.1f;
                Lighting.AddLight(this.position + new Vector2(8f, 8f), 1f * num8, 0.5f * num8, 0.1f * num8);
              }
            ...
            else if (this.sticky)
            {
              ...
              if (TextureAssets.Gore[this.type].IsLoaded)
              {
                num9 = TextureAssets.Gore[this.type].Width();
                if (TextureAssets.Gore[this.type].Height() < num9)
                  num9 = TextureAssets.Gore[this.type].Height();
              }
              if (flag1)
                num9 = 4;
              int num10 = (int) ((double) num9 * 0.89999997615814209);
              Vector2 velocity = this.velocity;
              this.velocity = Collision.TileCollision(this.position, this.velocity, (int) ((double) num10 * (double) this.scale), (int) ((double) num10 * (double) this.scale));
              if ((double) this.velocity.Y == 0.0)
              {
                if (flag1)
          ...
          if (this.type >= 907 && this.type <= 909)
          {
            ...
            if (TextureAssets.Gore[this.type].IsLoaded)
            {
              num11 = TextureAssets.Gore[this.type].Width();
              if (TextureAssets.Gore[this.type].Height() < num11)
                num11 = TextureAssets.Gore[this.type].Height();
            }
            int num12 = (int) ((double) num11 * 0.89999997615814209);
            Vector4 vector4 = Collision.SlopeCollision(this.position, this.velocity, num12, num12, fall: true);
    ...
    private void Gore_UpdateLeaf()
    {
      ...
      if ((double) this.velocity.Y < 0.0)
      {
        ...
        int num1 = (int) (4.0 * 0.89999997615814209);
      ...
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            this.rotation = vector2_3.ToRotation() + 1.57079637f;
            if (this.timeLeft > 0)
              this.timeLeft -= GoreID.Sets.DisappearSpeed[this.type];
            else
              this.alpha += GoreID.Sets.DisappearSpeedAlpha[this.type];
    ...
    private void Gore_UpdateSail()
    {
      ...
      else
      {
        ...
        if (v != vector2)
          this.velocity.Y = -1f;
        this.position += v;
        this.rotation = v.ToRotation() + 3.14159274f;
        if (this.timeLeft > 0)
          --this.timeLeft;
        else
          ++this.alpha;
    ...
    public static int NewGore(Vector2 Position, Vector2 Velocity, int Type, float Scale = 1f)
    {
      ...
      if (num1 == 1)
        Main.gore[index1].velocity = new Vector2((float) (((double) Main.rand.NextFloat() - 0.5) * 3.0), Main.rand.NextFloat() * 6.28318548f);
